...
cd $SCRIPT_DIR/..
PROJECT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )" #gets the current directory
...
rocker --nvidia --x11 --user --home --pull --git --name $CONTAINER_NAME --oyr-run-arg " --detach" ubuntu:22.04 


code --folder-uri vscode-remote://attached-container+$container_hex_id/$PROJECT_DIR